...
import { ContentItem, Data, Video } from '../../../../interfaces/model';
...
export class ListComponent implements OnInit {
  ...
  //   });
  // }
  getParcourData(idParcour: number ) {
    this.appService.url = `${environment.apiBaseUrl}parcour-by-id/${idParcour}`;
    ...
    this.appService.all().subscribe({
      next: (response: { data: Data }) => {
        ...
        console.log("Données du parcours:", parcour);
      //  console.log("Vidéos associées:", videos);

        // Retourner les données
        //return { idParcour: parcour.id, parcour, videos };
      },
      error: (err) => {
